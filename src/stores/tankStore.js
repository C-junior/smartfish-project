...
export const useTankStore = defineStore('tank', () => {
  ...
  const warningAlerts = computed(() => {
    return alerts.value.filter(alert => alert.severity === 'warning' && !alert.acknowledged)
  })
  ...
  const setupRealtimeListener = () => {
    try {
      ...
      unsubscribe = onSnapshot(tankRef, (docSnap) => {
        if (docSnap.exists()) {
  ...
  const createInitialTankData = () => {
    ...
    console.log('Tank document does not exist. Please ensure the document is created in Firestore.')
  }
      ...
      if (status === 'critical' || status === 'warning') {
        ...
        if (!existingAlert) {
          const alert = {
            ...
            sensor: sensorType,
            currentValue: value,
            threshold: status === 'critical' ? threshold.critical : (value > threshold.max ? threshold.max : threshold.min),
